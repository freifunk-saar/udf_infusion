...
DLLEXPORT my_bool percentile_disc_compat_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
    struct Buffer *data;
...
DLLEXPORT void percentile_disc_compat_clear(UDF_INIT* initid, char* is_null, char *error) {
    struct Buffer *data = (struct Buffer *) initid->ptr;
    array_truncate(&data->values);
}

DLLEXPORT void percentile_disc_compat_add(UDF_INIT* initid, UDF_ARGS* args, char* is_null, char *error) {
    struct Buffer *data = (struct Buffer *) initid->ptr;
...
DLLEXPORT void percentile_disc_compat_deinit(UDF_INIT *initid) {
    struct Buffer *data = (struct Buffer *) initid->ptr;
    if (NULL != data) {
...
DLLEXPORT double percentile_disc_compat(UDF_INIT *initid, UDF_ARGS *args,
        char *is_null,
